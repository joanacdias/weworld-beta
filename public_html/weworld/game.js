class Game {
  constructor() {
    ...
    this.characterOptionsList = ['girl1', 'girl2', 'girl3'],
    this.animationsList = ['idle', 'walking', 'walking_backwards', 'running', 'turn_left', 'turn_right', 'pointing_gesture'],

    this.anims = [];

    this.characterOptionsList.forEach(character => {
      this.animationsList.forEach(animation => {
        this.anims.push(`${character}_${animation}`);
      })
    })
  ...
  playerControl(forward, turn) {
    ...
    if (forward > 0.3) {
      if (this.player.action != this.player.model + "_walking" && this.player.action != this.player.model + "_running")
        this.player.action = this.player.model + "_walking";
    } else if (forward < -0.3) {
      if (this.player.action != this.player.model + "_walking_backwards")
        this.player.action = this.player.model + "_walking_backwards";
    } else {
      forward = 0;
      if (Math.abs(turn)>0.1){
        if (turn > 0 && this.player.action != this.player.model + "_turn_left") this.player.action = this.player.model + "_turn_left";
        if (turn < 0 && this.player.action != this.player.model + "_turn_right") this.player.action = this.player.model + "_turn_right";
      } else if (this.player.action != this.player.model + "_idle") {
        this.player.action = this.player.model + "_idle";
      }
    }
  ...
  animate() {
    ...
    if (this.player.action == this.player.model + "_walking") {
      const elapsedTime = Date.now() - this.player.actionTime;
      if (elapsedTime > 1000 && this.player.motion.forward > 0) {
        this.player.action = this.player.model + "_running";
      }
    }
...
class Player {
  constructor(game, options) {
    ...
    loader.load(`${game.assetsPath}fbx/anims/${model}_idle.fbx`, function (object) {
      ...
      if (game.animations[`${player.model}_idle`] !== undefined) player.action = player.model + "_idle";
    });
  }
  ...
  move(dt) {
    ...
    if (!blocked) {
      if (this.motion.forward > 0) {
        const speed = this.action == this.model + "_running" ? 500 : 150;
